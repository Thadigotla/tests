...
function App() {
  ...
  return (
    <div>
      ...
      >
        <button id="darkMode" onClick={createdarkMode}>{`${
          darkMode ? "Enable DarkMode" : "Disable DarkMode"
        } `}</button>
        {cUser ? (
          <CreateUser setcUser={createUsers} setRefetch={setRefetch} />
        ) : (
          <button onClick={() => setCUser(!cUser)}>Create</button>
        )}
